...
async function setupHands(){
  if(!window.Hands) throw new Error('ระบบตรวจจับมือโหลดไม่สำเร็จ กรุณาเชื่อมต่ออินเทอร์เน็ตแล้วรีโหลดหน้า');
...
async function startLoop(){
  ...
  const loop=async now=>{
    ...
    updateAnswers(dt);draw();handleFist(now);
    if(video.readyState>=2 && hands) await hands.send({image:video});
    raf=requestAnimationFrame(loop);
  };
  raf=requestAnimationFrame(loop);
}
function handleFist(now){
  if(!fist){lastFistState=false;fistSince=0;return}
  if(!lastFistState)fistSince=now;
  lastFistState=true;
  if(readyToCapture && now-fistSince>520 && selectedCandidate>=0){
    readyToCapture=false;submitAnswer(selectedCandidate);
  }
}
